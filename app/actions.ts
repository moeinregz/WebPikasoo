...
import { isValidPhone, normalizePhone, toEnglishDigits } from "@/lib/auth";
...
export async function submitInquiry(
  ...
  // Accept Persian/Arabic-Indic digits (۰۹۱۲... typed from a Persian
  // keyboard) exactly like plain ASCII ones: normalize to the standard
  // 09xxxxxxxxx form when it's a full valid number, otherwise still fall
  // back to converting just the digits so nothing Persian ever gets
  // silently mangled or rejected.
  const phone = phoneRaw ? (isValidPhone(phoneRaw) ? normalizePhone(phoneRaw) : toEnglishDigits(phoneRaw)) : "";
