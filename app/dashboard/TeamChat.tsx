    ...
    <div className="flex h-[560px] flex-col overflow-hidden rounded-card border border-ink/[0.14]">
      ...
      <form action={handleSubmit} className="flex flex-col gap-2 border-t border-ink/[0.14] bg-canvas p-4">
        ...
        <div className="flex items-center gap-2">
          {/* Attaching a file or recording a voice note is admin-only — for
              anyone else these controls don't even render (the server
              double-checks this too, in sendTeamMessageAction, so this is
              just about not showing a control that would be rejected). */}
          {isAdmin && (
            <>
              <input ref={fileInputRef} type="file" onChange={handleFileChange} className="hidden" id="chat-file-input" />
              <label
                htmlFor="chat-file-input"
                title="پیوست فایل (فقط ادمین)"
              ...
              <button
                ...
                title={recording ? "پایان ضبط" : "پیام صوتی (فقط ادمین)"}
