...
export async function sendTeamMessageAction(formData: FormData): Promise<DashboardFormState> {
  ...
  if (file instanceof File && file.size > 0) {
    // Only admins can attach a file (or voice note) to a chat message —
    // re-checked here server-side, since the client just hides the
    // attach/mic buttons for everyone else and that's only a convenience.
    if (!user.isAdmin) {
      return { ok: false, message: "فقط ادمین می‌تونه فایل یا پیام صوتی بفرسته." };
    }
    try {
