...
const HEADING_BUTTON_CLASS =
  "rounded-md border border-ink/[0.16] bg-canvas px-2 py-1.5 text-[12px] font-bold text-ink transition hover:border-accent hover:text-accent";

// Mirrors lib/uploads.ts's MAX_BLOG_IMAGE_BYTES — kept as a plain number
// here (not imported) since lib/uploads.ts pulls in server-only packages
// that shouldn't end up in the client bundle. Checking client-side just
// saves a round trip; the server enforces the real limit regardless.
const MAX_IMAGE_BYTES = 200 * 1024;
const MAX_IMAGE_LABEL = "۲۰۰ کیلوبایت";

function tooLargeMessage(file: File) {
  const kb = Math.round(file.size / 1024);
  return `حجم فایل (${kb} کیلوبایت) بیشتر از حد مجازه — حداکثر ${MAX_IMAGE_LABEL}.`;
}

/** Small popover for turning a selected word (or, if nothing was selected,
 *  a typed-in label) into a link: type/paste the URL and confirm, instead
 *  of hand-editing `[text](url)` syntax in the raw textarea. The word
 *  itself shows up in the accent color once the article is rendered on the
 *  public page (see lib/blogContent.tsx's renderInline), which is the
 *  whole point — it just can't be color-previewed inside a plain
 *  <textarea>, so this confirms it in words instead. */
function LinkPopover({
  ...
  function submit() {
    ...
    onConfirm(trimmedLabel, trimmedUrl);
  }

  return (
    <div className="mb-2 rounded-[10px] border border-accent/40 bg-accent/[0.06] p-3">
      {selectedText ? (
        <p className="mb-2 text-[12.5px] text-ink">
          لینک روی «<span className="font-bold text-accent">{selectedText}</span>» اضافه می‌شه:
        </p>
      ) : (
        <div className="mb-2">
          <label className="mb-1 block text-[12px] font-semibold text-dim">متنی که لینک می‌شه</label>
          <input
            value={label}
            onChange={(e) => setLabel(e.target.value)}
            placeholder="مثلاً: صفحه‌ی نمونه‌کارها"
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input
          ...
          placeholder="https://..."
          autoFocus
          className={`${inputClass} flex-1 py-2 text-left`}
        />
        <button
          type="button"
          onClick={submit}
          disabled={!url.trim() || !label.trim()}
          className="rounded-full bg-accent px-4 py-2 text-[12.5px] font-bold text-black transition hover:-translate-y-0.5 disabled:pointer-events-none disabled:opacity-50"
        >
          افزودن لینک
        </button>
        <button
      ...
      <p className="mt-1.5 text-[11.5px] text-dim/70">
        بعد از انتشار، این کلمه به رنگ آبی و زیرخط‌دار (به‌عنوان لینک) نمایش داده می‌شه.
      </p>
    </div>
  );
}
...
function ContentEditor({ defaultValue }: { defaultValue?: string }) {
  ...
  const [linkPopover, setLinkPopover] = useState<{ start: number; end: number; text: string } | null>(null);
  ...
  /** H2 down to H6 — level 2 is the biggest section title, level 6 the
   *  smallest, matching how lib/blogContent.tsx sizes them on the article
   *  page (and hides the "#" markers themselves — those never show up
   *  rendered, they're just the editor's shorthand for "this is a heading"). */
  function handleHeading(level: 2 | 3 | 4 | 5 | 6) {
  ...
  function handleLinkClick() {
    ...
    setLinkPopover({ start, end, text: value.slice(start, end) });
  }

  function applyLink(label: string, url: string) {
  ...
  return (
    <div>
      ...
      <div className="mb-2 flex flex-wrap items-center gap-1.5">
        <div className="flex items-center gap-1 rounded-md border border-ink/[0.1] p-0.5">
        ...
        <button
          ...
          title={`افزودن عکس داخل مقاله (حداکثر ${MAX_IMAGE_LABEL})`}
        >
      ...
      <p className="mt-1 text-[12px] text-dim/70">
        متن رو انتخاب کن و «لینک» رو بزن — یه کادر باز می‌شه که آدرس لینک رو توش وارد می‌کنی؛ بدون انتخاب متن هم
        می‌تونی خودِ متنِ لینک رو تو همون کادر بنویسی. عکس‌ها حداکثر {MAX_IMAGE_LABEL} می‌تونن باشن.
      </p>
    </div>
  );
}
...
function BlogFields({
  ...
  function handleCoverImageChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file && file.size > MAX_IMAGE_BYTES) {
      setCoverImageError(tooLargeMessage(file));
      e.target.value = ""; // drop the oversized file so it can't be submitted
    } else {
      setCoverImageError("");
    }
  }
  ...
  return (
    <div className="grid gap-4">
      ...
      <div>
        ...
        />
        <p className="mt-1 text-[12px] text-dim/70">حداکثر حجم: {MAX_IMAGE_LABEL}.</p>
        {coverImageError && <p className="mt-1 text-[12px] text-red-500">{coverImageError}</p>}
        {defaults?.coverImage && (
