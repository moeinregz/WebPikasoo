...
// 200KB — covers both the post cover image and in-article images (same
// helper saves both). Exported so the dashboard's blog editor can reject an
// oversized file instantly, client-side, before ever uploading it.
export const MAX_BLOG_IMAGE_BYTES = 200 * 1024;
...
export async function saveBlogImage(file: File): Promise<string | null> {
  ...
  if (file.size > MAX_BLOG_IMAGE_BYTES) {
    throw new Error("حجم عکس بیشتر از حد مجازه (حداکثر ۲۰۰ کیلوبایت).");
  }
